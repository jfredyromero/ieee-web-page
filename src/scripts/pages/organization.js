import juntaDirectiva from "../../assets/img/pages/organization/junta-directiva.svg";
import comites from "../../assets/img/pages/organization/comites.svg";
import { API_DOMAIN } from "../utilities/api-domain";

export default () => {
	document.getElementById("junta-directiva-img").src = juntaDirectiva;
	document.getElementById("comites-img").src = comites;
	...
	juntaDirectivaEvents.forEach((dot, index) => {
		dot.addEventListener("click", () => {
			...
			juntaDirectivaImage[index].classList.add(
				"juntadirectiva__active-card"
			);
	...
	function displayMemberData() {
		...
		document.querySelector(".juntadirectiva__p").innerHTML =
			datosMiembro.frase;
		document.querySelector(`[data-card=${datosMiembro.cargo}]`).src =
			datosMiembro.urlFoto;
	}

	function getMemberData(apiUrl) {
		fetch(API_DOMAIN + apiUrl)
